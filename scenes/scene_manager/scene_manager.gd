...
const PATH_TO_LEVEL_LIST : String = "res://scenes/level/levels"
...
func load_level_by_number(level_number : int, parent_scene : Node):
	for level in level_scenes:
		var current_level_name_formatted : String = _remove_ending_after_marking_from_file_name(level.get_path(), "/", 5)
...
func load_scene_by_name(scene_name : String, scene_type : SceneType, parent_scene : Node):
	...
	if scene_type == SceneType.Level:
		for level in level_scenes:
			var current_level_name_formatted = _remove_ending_after_marking_from_file_name(level.get_path(), "/", 5)
		...
		for ui in ui_scenes:
			var current_ui_name_formatted = _remove_ending_after_marking_from_file_name(ui.get_path(), "/", 5)
